...
#ifndef _STDCORELIB_LLVM_ADT_SMALLVECTOR_H
#define _STDCORELIB_LLVM_ADT_SMALLVECTOR_H
...
protected:
  ...
  bool isSafeToReferenceAfterResize(const void *Elt, size_t NewSize) {
    // Past the end.
    if (STDCORELIB_LIKELY(!isReferenceToStorage(Elt)))
      return true;
  ...
  static const T *reserveForParamAndGetAddressImpl(U *This, const T &Elt,
    ...
    if (STDCORELIB_LIKELY(NewSize <= This->capacity()))
      return &Elt;

    bool ReferencesStorage = false;
    int64_t Index = -1;
    if (!U::TakesParamByValue) {
      if (STDCORELIB_UNLIKELY(This->isReferenceToStorage(&Elt))) {
...
public:
  ...
  template <typename... ArgTypes> reference emplace_back(ArgTypes &&... Args) {
    if (STDCORELIB_UNLIKELY(this->size() >= this->capacity()))
...
#endif // _STDCORELIB_LLVM_ADT_SMALLVECTOR_H